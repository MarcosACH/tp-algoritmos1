...
public class Dimension {
    ...
    public String toString() {
        return "Dimension " + nombre + ", niveles=" + valoresToCeldas.size() + ", (nivel actual = "
                + (valoresToCeldas.size() - nivelActual) + ")";
    }
     ...
     * @param indiceCelda Indice de la celda en el cubo
        ...
        for (int j = 0; j < valoresToCeldas.size(); j++) {
            if (!idToValores.get(j).containsKey(idValor)) {
                throw new IllegalArgumentException(
                        "El id " + idValor + " del valor no existe en la dimension " + nombre + "en el nivel" + j);
            }
        }
        for (int i = 0; i < valoresToCeldas.size(); i++) {
    ...
    public void aumentarJerarquia() {
        if (nivelActual > 0) {
            ...
            throw new IllegalStateException(
                    "No se puede aumentar mas el nivel de jerarquia de la dimensión: " + nombre);
        }
    }
     ...
     * 
     * @throws IllegalStateException Si nivelActual es igual a la cantidad de
     *                               jerarquias que tiene la dimensión menos uno
     */
    public void disminuirJerarquia() {
        if (nivelActual < valoresToCeldas.size() - 1) {
            ...
            throw new IllegalStateException(
                    "No se puede disminuir mas el nivel de jerarquia de la dimensión: " + nombre);
        }
    }
}
